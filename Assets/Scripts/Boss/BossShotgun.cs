...
public class BossShotgun : MonoBehaviour
{
    ...
    [SerializeField] Level3Manager _bossController;
    ...
    void Move()
    {
        transform.position = Vector2.MoveTowards(transform.position, _patrolPoints[currentDestination].position, _speedMovement * Time.deltaTime);
    }

    void Attack()
    {
        if (_attackCooldown >= 0.8f)
        {
            for (int i = 0; i < 3; i++)
            {
